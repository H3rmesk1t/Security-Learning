...
在`PHP`中，可以利用`PHP Base64 Filter`宽松的解析，结合`iconv filter`等编码组合来构造特定的`PHP`代码完成无需临时文件的`RCE`

但是需要注意，这种`tips`会受到`iconv`的限制，如果系统没有`iconv`或者不同的`iconv`版本，将会有不同的表现

# 分析

`PHP Base64 Filter`的特性利用在`P牛`提出来的关于死亡`exit`的绕过手法中就已经出现，对于`PHP Base64 Filter`来说，解码过程中会忽略掉非正常编码的字符，例如`<?php exit; ?>`，在使用`php://filter/write=convert.base64-decode`进行解码时，字符`<`、`>`、`?`、 ``和`;`由于不在`base64`编码的字符范围（`A-Za-z0-9\/\=\+`）中，会被自动忽略掉，因此最终被解码的字符只有`phpexit`以及后续传入的其它可解码字符

在`PHP Filter`中有一种`convert.iconv`的`Filter`，可以用来将数据从字符集`A`转换为字符集`B`，其中这两个字符集可以从`iconv -l`获得

![Untitled](LFI%20In%20PHP%20Base64%20Filter%2018b41d18fde549a9ac96c69a840c4eca/Untitled.png)

示例代码中，利用`iconv`来将`UTF-8`字符集转换到`UTF-7`字符集，结合`PHP Base64 Filter`的特性，可以利用某些固定文件内容来构造需要的内容
...
假定文件的内容为`aaaaaaaaaaaaaa`，通过暴力遍历`iconv`支持的字符编码方式，利用`convert.base64-decode`去掉产生的不可见字符，再用`convert.base64-encode`还原可见字符，来尝试获取需要的内容

可以看到，通过暴力遍历的方式，成功`Fuzz`到字符`C`
...
for ($i = 0; $i < count($iconv_array); $i++) {
    ...
    $url .= $iconv_array[$i];
    $url .= "|convert.base64-decode|convert.base64-encode/resource=data:,hhhhhhhhhhhhhh";
    echo "[+] " . $iconv_array[$i] . "  ====>   ";
...
由于在`base64`编码的字符范围中并不包含`<`、`?`等字符，所以无法直接构造出可用的`Webshell`，但是可以利用上述方式来构造一个`base64`字符串，最后再调用一次`convert.base64-decode`进行解码就可以利用`Webshell`

因此，现在只需要利用上述的编码技巧来寻找能够构造出`Webshell`的`base64`字符串即可，这里已经有了现成的`Fuzz`代码：[https://github.com/wupco/PHP_INCLUDE_TO_SHELL_CHAR_DICT](https://github.com/wupco/PHP_INCLUDE_TO_SHELL_CHAR_DICT)
